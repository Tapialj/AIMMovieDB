...
const DirectorForm = ({
  director = { id: null, firstName: "", lastName: "" },
  movies = [],
  directed = [],
  isModal = false,
  onSaveDirector,
}) => {
  const [selectedMovies, setSelectedMovies] = useState(directed);
  const [firstName,setFirstName] = useState(director.firstName);
  const [lastName,setLastName] = useState(director.lastName);
  ...
  const saveDirector = async () => {
    try {
      const diff = selectedMovies.filter((movie) => !directed.some((m) => m.id === movie.id));
      const removedDiff = directed.filter((movie) => !selectedMovies.some((m) => m.id === movie.id));
      const directorApi = director.id != null ? `api/directors/${director.id}` : "api/directors";
      let savedDirector;
      ...
      if(location.pathname.includes("directors/new") || location.pathname.includes("movies/new")) {
        ...
        const res = await axiosPrivatePut(directorApi, { id: director.id, ... directorObject });
        savedDirector = res?.data;
      }
      
      for(const addMovie of diff) {
        await axiosPrivatePost(`api/directors/movies/${addMovie.id}`, savedDirector);
      }

      for(const removeMovie of removedDiff) {
        await axiosPrivateDelete(`api/directors/${savedDirector.id}/movies/${removeMovie.id}`);
      }

      if(!isModal)
        navigate(-1, { replace: true });
      else
        onSaveDirector(savedDirector);
    }
    catch(e) {
      console.log("error", e);
      if(!e?.response) {
  ...
  return (
    <>
      ...
      <form className="form" noValidate>
        ...
        <Button title="Save Director" onClick={onSave} />
      </form>
    </>
  );
};
